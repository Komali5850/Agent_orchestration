...
The backend (Flask + LangChain + Groq Llama‑3.3‑70B‑Versatile) coordinates specialized agents for symptoms, lifestyle, diet, and fitness, then synthesizes a safe wellness plan. The frontend (React + Vite) provides a guided experience for login, profile management, wellness queries, and follow-up questions.
...
```json
{
	"synthesized_guidance": "# Markdown wellness plan...",
	"recommendations": ["..."],
	"raw_agent_outputs": {
		"symptom_analysis": {},
		"lifestyle": {},
...
- **Follow-up Q&A**
	...
	- Backend uses the last stored plan + recommendations as context to ensure continuity and safer responses
...
- Custom CSS for cards, navbar, buttons, responsive layout
...
Returns only the recommendations as bullet points.
...
From the repo root (backend at root):
...
- The project is intended for educational/demo purposes and is not a replacement for medical advice.
